...
export const allVocab = days.flatMap((d) =>
  (d.vocabulary || []).map((v) => ({ ...v, day: d.id, week: d.week, topic: d.title, layout: d.vocabLayout || 'compact' }))
);

/** Group by topic for the Vocabulary page. */
export const vocabByTopic = days
  .filter((d) => (d.vocabulary || []).length)
  .map((d) => ({ day: d.id, week: d.week, title: d.title, emoji: d.emoji, layout: d.vocabLayout || 'compact', items: d.vocabulary }));
