 ...
 * intro → run each of the four modules (timed) with a short break between →
...
export default function Mock() {
  ...
  const finishModule = (r) => {
    ...
    setResults((prev) => {
      ...
      if (modIdx + 1 >= MOCK_MODULES.length) setPhase('done');
      else setPhase('break');
      return next;
    });
  };
    ...
    return (
      <div className="space-y-5 max-w-lg mx-auto">
        <div className="card text-center">
          ...
          <h1 className="text-2xl font-extrabold">Mock exam complete</h1>
  ...
  if (phase === 'break') {
    const next = MOCK_MODULES[modIdx + 1];
    ...
    return (
      <div className="card text-center max-w-md mx-auto">
        ...
        <p className="text-xs text-slate-400 mt-1">about {next?.minutes} minutes</p>
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold mt-1">🏁 Full mock exam</h1>
        <p className="text-slate-500 mt-1">
          The whole Goethe A1 in one sitting: all four skills, back to back, each timed. You’ll get one overall
          score and a per-skill breakdown. Pass mark: <strong>60%</strong>.
        </p>
      </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm text-slate-500">~{MOCK_TOTAL_MINUTES} min total</span>
        </div>
        <ol className="mt-3 space-y-2">
          {MOCK_MODULES.map((m, i) => (
            <li key={m.key} className="flex items-center gap-3">
              ...
              <span className="ml-auto text-xs text-slate-400 tabular-nums">{m.minutes} min</span>
            </li>
          ))}
        </ol>
      </div>
      ...
      <button className="btn-primary w-full text-lg py-4" onClick={start}>Start mock exam →</button>
    </div>
  );
}
